#BiGRU Attention
# ============================================================
# BiGRU + ATTENTION EXTRACTIVE QA FOR AMHARIC
# ============================================================
...
# -------------------------------
# 6. BiGRU + ATTENTION MODEL
# -------------------------------
class BiGRUAttentionQAModel(nn.Module):
    def __init__(self, embedding_matrix, hidden_size=128, dropout=0.3):
        ...
        # Bidirectional GRU for context and question
        # dropout applies between GRU layers if num_layers > 1
        self.gru_context = nn.GRU(
            emb_dim, hidden_size, batch_first=True,
            bidirectional=True, dropout=dropout
        )
        self.gru_question = nn.GRU(
            emb_dim, hidden_size, batch_first=True,
            bidirectional=True, dropout=dropout
        )

        # Output layer: input size = hidden_size * 4
        # (concatenation of context (2*hidden) and attended question (2*hidden))
    ...
    def _init_weights(self):
        for name, param in self.gru_context.named_parameters():
            if 'weight_ih' in name:
                nn.init.xavier_uniform_(param)
            elif 'weight_hh' in name:
                nn.init.orthogonal_(param)
        for name, param in self.gru_question.named_parameters():
            if 'weight_ih' in name:
                nn.init.xavier_uniform_(param)
            elif 'weight_hh' in name:
                nn.init.orthogonal_(param)
    ...
    def forward(self, context, question):
        ...
        # BiGRU forward
        ctx_out, _ = self.gru_context(ctx_emb)   # (batch, ctx_len, 2*hidden)
        q_out, _   = self.gru_question(q_emb)    # (batch, q_len, 2*hidden)
...
# 8. TRAINING LOOP
# -------------------------------
model = BiGRUAttentionQAModel(embedding_matrix, hidden_size=HIDDEN_SIZE, dropout=DROPOUT).to(DEVICE)
